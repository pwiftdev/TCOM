...
const schema = z.object({ name: z.string().min(3).max(80), slug: z.string().min(3).max(80).optional(), description: z.string().max(5000).optional().default(''), visibility: z.enum(['public', 'private', 'invite']).default('public'), tags: z.array(z.string()).optional().default([]) });

router.get('/', async (_req, res) => {
  const { data, error } = await supabase.from('communities').select('*').eq('visibility', 'public').order('member_count', { ascending: false }).limit(30);
  if (error) return res.status(400).json({ error: error.message });
  return res.json(data);
});
router.get('/:slug', async (req, res) => {
  const { data, error } = await supabase.from('communities').select('*').eq('slug', req.params.slug).single();
  if (error) return res.status(404).json({ error: 'Community not found' });
  return res.json(data);
});
router.post('/', authenticate, async (req, res) => {
  ...
  const { data: community, error } = await supabase.from('communities').insert({ ...input, slug, owner_id: req.user.id, member_count: 1 }).select('*').single();
  if (error) return res.status(400).json({ error: error.message });
...
router.post('/:slug/join', authenticate, async (req, res) => {
  ...
  await supabase.from('community_members').upsert({ community_id: community.id, user_id: req.user.id, role: 'member' }, { onConflict: 'community_id,user_id' });
